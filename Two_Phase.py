...
class LP_model_solver(object):
    ...
    def _simplex_tableau(self, text_widget):  # Pass in the text widget
        ...
        # Usar la regla de Bland para seleccionar la variable entrante
            ...
            enter_axis_idx = min(positive_indices)  # Elegir el índice de menor valor
        ...
        # Usar la regla de Bland para seleccionar la variable saliente
            ...
            leave_axis_idx = min(leave_candidates, key=lambda x: (x[1], self.basis[x[0]]))[0]
        ...
        # Display each iteration result in the Text widget
            text_widget.insert(tk.END, f"=== Iteration #{iteration} ===\n")
    ...
    # Mostrar la última tabla de iteración antes de finalizar
        self._display_tableau(text_widget, iteration)
